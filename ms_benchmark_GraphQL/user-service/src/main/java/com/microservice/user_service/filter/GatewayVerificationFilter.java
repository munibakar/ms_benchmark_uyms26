...
public class GatewayVerificationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (requestURI.startsWith("/actuator/health") || requestURI.equals("/api/users/health")) {
            log.debug("Health check request - bypassing gateway verification");
            chain.doFilter(request, response);
            return;
        }

        // GraphQL endpoint için bypass (microservice-to-microservice communication)
        if (requestURI.startsWith("/graphql") || requestURI.startsWith("/graphiql")) {
            log.debug("GraphQL request - bypassing gateway verification for inter-service communication");
            chain.doFilter(request, response);
            return;
        }
