package com.microservices.content_management_service.filter;
...
public class GatewayVerificationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Actuator health endpoint'leri için bypass (Docker healthcheck için gerekli)
        if (requestURI.startsWith("/actuator/health") || requestURI.equals("/api/contents/health")) {
            log.debug("Health check request - bypassing gateway verification");
            chain.doFilter(request, response);
            return;
        }

        // GraphQL endpoint'leri için bypass (microservice-to-microservice iletişim için)
        if (requestURI.startsWith("/graphql") || requestURI.startsWith("/graphiql")) {
            log.debug("GraphQL request - bypassing gateway verification");
            chain.doFilter(request, response);
            return;
        }
